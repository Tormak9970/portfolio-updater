<script lang="ts">
	...
	function handleInput(e:Event) { value = (e.currentTarget as HTMLInputElement).value; }
...
<div class="image-input">
    ...
    <input bind:this="{input}" type="text" placeholder="{cVal}" on:change="{handleInput}">
...
<style>
	...
	.image-input > .field-name { margin-right: 10px; }
    ...
    .image-input > input:hover { outline: 1px solid var(--highlight-hover); }
    .image-input > input:focus { outline: 1px solid var(--highlight); }
    ...
    .image-input > .upload-file:focus { outline: 1px solid var(--highlight); }
</style>